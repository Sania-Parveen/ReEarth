...
const wasteTypes = {
  battery: {
    ...
    avg_weight_g: 50,
    recycling_methods:
      "Batteries contain hazardous materials and valuable metals. They are collected and processed in specialized facilities to recover materials and neutralize harmful substances.",
  },
  biological: {
    ...
    avg_weight_g: 150,
    recycling_methods:
      "Composted through natural decomposition, often in industrial facilities or home compost bins. The resulting compost enriches soil and reduces methane emissions from landfills.",
  },
  cardboard: {
    label: "Cardboard",
    ...
    avg_weight_g: 100,
    recycling_methods:
      "Similar to paper, cardboard is pulped, cleaned, and then reformed into new cardboard boxes or other paper products. It's highly recyclable and reduces the need for new timber.",
  },
  clothes: {
    label: "Clothes",
    ...
    avg_weight_g: 300,
    recycling_methods:
      "Reusable clothes can be donated. Unwearable textiles are often shredded and used for insulation, wiping rags, or reprocessed into new fibers for various products.",
  },
  glass: {
    label: "Glass",
    color: "text-sky-600",
    ...
    avg_weight_g: 250,
    recycling_methods:
      "Sorted by color, crushed into cullet, cleaned, and then melted in furnaces to create new glass bottles, jars, or fiberglass. Glass can be recycled endlessly without loss of quality.",
  },
  metal: {
    label: "Metal",
    color: "text-blue-400",
    ...
    avg_weight_g: 100,
    recycling_methods:
      "Metals like aluminum and steel are highly recyclable. They are melted down and reformed into new products, saving significant energy and raw materials compared to producing from virgin ores.",
  },
  paper: {
    label: "Paper",
    color: "text-red-600",
    ...
    avg_weight_g: 50,
    recycling_methods:
      "Sorted, pulped with water, cleaned to remove impurities, and then pressed and dried to form new paper products like cardboard boxes, newspapers, or tissue paper.",
  },
  plastic: {
    label: "Plastic",
    ...
    avg_weight_g: 20,
    recycling_methods:
      "Typically collected, sorted by type (PET, HDPE), cleaned, shredded into flakes, melted, and then formed into new plastic products like bottles, fibers, or containers.",
  },
  shoes: {
    label: "Shoes",
    ...
    avg_weight_g: 400,
    recycling_methods:
      "Depending on condition, shoes can be donated for reuse. Worn-out shoes can be recycled for their rubber, foam, and fabric components, often used in playground surfaces or new footwear.",
  },
  trash: {
    label: "Trash",
    color: "text-gray-600",
    ...
    avg_weight_g: 100,
    recycling_methods:
      "Typically sent to landfills or incinerated. Proper disposal is crucial to minimize environmental impact, though it offers fewer resource recovery benefits compared to recycling or composting.",
  },
};
...
function WastePredictor() {
  ...
  const capture = useCallback(() => {
    ...
    if (!selectedImage) {
      ...
      setSelectedImage(
        "https://placehold.co/600x400/000000/FFFFFF?text=Simulated+Capture"
      );
  ...
  const handleScanClick = async () => {
    if (!selectedFile && !selectedImage) {
      // Ensure an image is selected/captured
    ...
    try {
      ...
      if (wasteTypes[detectedWasteTypeKey]) {
        // Check if the key exists in our wasteTypes object
        const detectedItem = {
          id: Date.now(),
          ...
          weight_g:
            wasteWeight !== ""
              ...
              : (
                  wasteTypes[detectedWasteTypeKey].avg_weight_g *
                  (0.8 + Math.random() * 0.4)
                ).toFixed(0),
        ...
        setErrorMessage(
          `Unknown waste type detected: "${result.class}". Please ensure your Flask class labels match the keys in frontend's wasteTypes object.`
        );
        setDetectionResult({
          id: Date.now(),
          ...
          weight_g:
            wasteWeight !== ""
              ? parseFloat(wasteWeight)
              : wasteTypes["trash"].avg_weight_g,
          location: location || "Unspecified Location",
        });
        setDetectedItems((prevItems) => [
          ...prevItems,
          { ...detectionResult, type: "trash" },
        ]);
      }
    } catch (error) {
      console.error("Error during prediction:", error);
      setErrorMessage(
        `Prediction failed: ${error.message}. Please ensure your Flask server is running and accessible.`
      );
  ...
  const currentWasteInfo = detectionResult
    ? wasteTypes[detectionResult.type]
    : null;
  ...
  const generateReuseIdeas = async () => {
    ...
    const apiKey = "AIzaSyAOwAOmAPt159Hyi8Q0ibFDWB0KQa58u6o"; // Canvas will automatically provide the API key at runtime
    ...
    try {
      ...
      setReuseIdeas([
        `Failed to generate ideas: ${errorDetail}. Please try again.`,
      ]);
    } finally {
      setLlmLoading(false);
    }
  ...
  const generateWasteReductionTips = async () => {
    ...
    try {
      ...
      const errorDetail =
        error instanceof Error ? error.message : JSON.stringify(error);
      setReductionTips([
        `Failed to generate tips: ${errorDetail}. Please try again.`,
      ]);
    } finally {
  ...
  const generateReport = async () => {
    ...
    const totalWeight = detectedItems.reduce(
      (sum, item) => sum + parseFloat(item.weight_g),
      0
    );
    ...
    let reportPrompt = `Generate a short, encouraging, and beautiful summary for a waste classification report. The user has classified ${totalItems} items today, with a total weight of ${totalWeight.toFixed(
      2
    )} grams.`;
    reportPrompt += ` They classified the following types: ${Object.entries(
      typeCounts
    )
      .map(([type, count]) => `${count} ${wasteTypes[type].label}(s)`)
      .join(", ")}.`;
    if (Object.keys(locationCounts).length > 0) {
      reportPrompt += ` Items were classified from locations such as: ${Object.entries(
        locationCounts
      )
    ...
    try {
      ...
      if (
        ...
        setReportMessage(
          "Could not generate report summary. Please try again."
        );
      }
    } catch (error) {
      ...
      const errorDetail =
        error instanceof Error ? error.message : JSON.stringify(error);
      setReportMessage(
        `Failed to generate report summary: ${errorDetail}. Please try again.`
      );
    } finally {
  ...
  return (
    <div
      className="relative w-full min-h-screen text-gray-800 flex flex-col lg:flex-row items-center justify-center overflow-hidden font-inter p-4"
    ...
    >
      ...
      */}
      ...
      <div className="relative z-10 flex flex-col items-center justify-center w-full lg:w-1/2 min-h-[50vh] lg:min-h-screen p-4 bg-white/70 lg:bg-transparent rounded-xl lg:rounded-none shadow-lg lg:shadow-none">
        <h2 className="text-3xl font-bold text-gray-800 mb-6 text-center lg:mt-0 mt-8">
          Waste Scanner
        </h2>
        ...
        <div className="relative w-full max-w-sm h-64 md:h-80 lg:h-96 rounded-xl overflow-hidden bg-white shadow-xl flex items-center justify-center border-4 border-green-300 mb-6 animate-pulse-border">
          {useCamera ? (
            <div className="w-full h-full flex items-center justify-center bg-gray-100">
              <p className="text-gray-500 text-lg md:text-xl animate-pulse">
          ...
          {scanning && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-70">
              ...
              <p className="text-white text-lg md:text-xl font-mono tracking-wider animate-pulse">
                SCANNING...
              </p>
            </div>
          )}
      ...
      <div className="relative z-10 flex flex-col items-center justify-center w-full lg:w-1/2 min-h-[50vh] lg:min-h-screen p-4 bg-green-200 backdrop-blur-sm lg:rounded-l-3xl shadow-2xl">
        ...
        {detectionResult && currentWasteInfo && (
          <div
            className={`p-6 rounded-xl shadow-2xl border-2 ${currentWasteInfo.color.replace(
              "text-",
              "border-"
            )}
          ...
          >
            {/* Waste type icon */}
            <div
              className={`text-5xl mb-3 animate-bounce-in-icon ${currentWasteInfo.color}`}
            >
              {currentWasteInfo.icon}
            </div>
            {/* Classified label */}
            <p
              className={`text-3xl md:text-4xl font-extrabold ${currentWasteInfo.color} mb-2`}
            >
            ...
            <div className="mt-4 text-gray-700 text-base md:text-lg">
              <p>
                🌍 Saved:{" "}
                <span className="font-semibold">
                  {currentWasteInfo.co2_saved_kg.toFixed(2)} kg CO2
                </span>
              </p>
              <p>
                ⚡ Saved:{" "}
                <span className="font-semibold">
                  {currentWasteInfo.energy_saved_kwh.toFixed(2)} kWh Energy
                </span>
              </p>
              <p>
                ⚖️ Weight:{" "}
                <span className="font-semibold">
                  {detectionResult.weight_g} g
                </span>
              </p>
              <p>
                📍 Location:{" "}
            ...
            <div className="mt-4 text-gray-700 text-sm md:text-base text-center">
              ...
              <p>
                Check your local municipality's website for specific collection
                schedules and accepted materials.
              </p>
              <p className="text-green-600 cursor-pointer hover:underline mt-1">
            ...
            <>
              <button
                ...
                style={{
                  ...
                  color: "#6a5acd" /* a darker lavender for text */,
                }}
              >
                Scan New Item
              </button>
          ...
          <div className="bg-white rounded-xl p-8 max-w-lg w-full shadow-2xl border border-green-300 relative">
            ...
            <div className="space-y-4 text-gray-700 max-h-96 overflow-y-auto pr-2 custom-scrollbar">
              <div>
                ...
                <p className="text-lg">
                  This item should be placed in the{" "}
                  <span className="font-bold text-green-500">
                    {currentWasteInfo.disposal}
                  </span>
                  .
              ...
              <div>
                ...
                <p className="text-lg">
                  By properly disposing of this item, you help save
                  approximately:
                </p>
                <ul className="list-disc list-inside ml-4 mt-2">
                  <li>
                    <span className="font-bold">
                      {currentWasteInfo.co2_saved_kg.toFixed(2)} kg of CO2
                      emissions
                    </span>{" "}
                    (equivalent to driving a car for X miles).
                  </li>
                  <li>
                    <span className="font-bold">
                      {currentWasteInfo.energy_saved_kwh.toFixed(2)} kWh of
                      energy
                    </span>{" "}
                    (enough to power a lightbulb for Y hours).
                  </li>
                </ul>
              </div>
              ...
              <div>
                ...
                <p className="text-lg">
                  Specific guidelines can vary by location. Please refer to your
                  local waste management authority for precise instructions.
                </p>
                <ul className="list-disc list-inside ml-4 mt-2">
                  <li>
                    <span className="font-bold">Preparation:</span>{" "}
                    {currentWasteInfo.label.includes("Bottle") ||
                    currentWasteInfo.label.includes("Jar")
                      ? "Rinse thoroughly and remove caps/lids."
                      : "Ensure item is clean and dry."}
                  </li>
                  <li>
                    <span className="font-bold">Collection:</span> Check your
                    municipal website for curbside pickup schedules.
                  </li>
                  <li>
                    <span className="font-bold">Drop-off:</span> Find nearby
                    recycling centers that accept {currentWasteInfo.label}s.{" "}
                    <a href="#" className="text-green-600 hover:underline">
                      Search local drop-off points
                    </a>
                  </li>
                </ul>
              </div>
              ...
              <div>
                ...
                <p className="text-lg leading-relaxed">
                  {currentWasteInfo.recycling_methods}
                </p>
              </div>
              ...
              {currentWasteInfo.reusable && (
                <div>
                  ...
                  <p className="text-lg">
                    This item has great potential for creative reuse! Click the
                    "✨ Chat" button on the sidebar screen for suggestions.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="bg-white rounded-xl p-8 max-w-lg w-full shadow-2xl border border-green-300 relative">
            ...
            {reportLlmLoading ? (
              <div className="flex flex-col items-center justify-center py-10">
                ...
                <p className="text-gray-700 text-lg">
                  Generating personalized report summary...
                </p>
              </div>
            ) : (
              <>
                  ...
                  <ul className="space-y-2">
                    {detectedItems.length > 0 ? (
                      detectedItems.map((item) => (
                        <li
                          key={item.id}
                          className="bg-gray-100 p-3 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center text-sm border border-gray-200"
                        >
                          <div>
                            <span
                              className={`font-bold ${
                                wasteTypes[item.type].color
                              }`}
                            >
                              {wasteTypes[item.type].label}
                            </span>
                      ...
                      <p className="text-center text-gray-500 py-4">
                        No items detected yet. Scan some waste!
                      </p>
                    )}
                  </ul>
                </div>
